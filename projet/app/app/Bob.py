...
class Bob:
    ...
    def fuck(self):
        ...
        for myLover in self.game.gridBob[self.coord] :
            ...
            #SI le rapport des masses est bon & si l'énergie des deux bobs est supérieur au seuil nécessaire pour la reproduction sexuelle et si le partenaire n'a pas déja effectué une action pendant ce tick.    
                ...
                babyBob.perceptionScore = round(self.Val_Mutation(TauxMutationPerception, (self.perceptionScore + myLover.perceptionScore)/2))
    ...
    def use_memory(self):
        ...
        while len(self.path) > 2 * (self.memory-int(self.remembering_food)) or self.memory < int(self.remembering_food):
            del self.path[0]
