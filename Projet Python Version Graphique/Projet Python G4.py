...
LOGIQUE_SIMPLE = True #Utiliser seulement la logique simple pour la démo.
...
### Grille ###
gridSizeX = 200 #Taille de la grille sur l'axe X.
gridSizeY = 200 #Taille de la grille sur l'axe Y. 
...
foodQty = 4000 #quantité d'items Food générés par jours
...
### Bob's parameters ###
#Bob's energy
energyInitLevel = 200 #Niveau d'énergie initial des Bobs.
    ...
    def fuck(self):
        ...
        for myLover in self.game.gridBob[self.coord] :
            ...
            #SI le rapport des masses est bon & si l'énergie des deux bobs est supérieur au seuil nécessaire pour la reproduction sexuelle et si le partenaire n'a pas déja effectué une action pendant ce tick.    
                ...
                babyBob.perceptionScore = self.Val_Mutation(TauxMutationPerception, (self.perceptionScore + myLover.perceptionScore)/2)
            ...
            if not bob.eat(): #Eat food
                if not bob.hunt(): #Hunt other bobs
                    if not bob.fuck(): #Fuck
                        if LOGIQUE_SIMPLE : bob.random_move_basic()
                        else : bob.move()
    ...
    def update_bobs(self):
        ...
        for key in self.Game.gridBob:
            for bob in self.Game.gridBob[key]:
                ...
                if bob.velocity >= 1.2 and bob.velocity < 1.6:
                    self.GBob_image = assets["bob.blue"]
                elif bob.velocity >= 1.6 :
