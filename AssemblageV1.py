...
### Grille ###
gridSizeX = 20 #Taille de la grille sur l'axe X.
gridSizeY = 20 #Taille de la grille sur l'axe Y. 
...
class Bob:
    ...
    def __init__(self, ID, coord) :
        ...
        self.image_bob = pygame.image.load("annexes/bob.png")
        self.image_bob_blue = pygame.image.load("annexes/bob.blue.png")
        self.image_bob_rouge = pygame.image.load("annexes/bob.rouge.png")
        self.image_bob = pygame.transform.scale(self.image_bob, (16, 16))
        self.image_bob_blue = pygame.transform.scale(self.image_bob_blue, (16, 16))
        self.image_bob_rouge = pygame.transform.scale(self.image_bob_rouge, (16, 16))
        self.rect = self.image_bob.get_rect()
    ...
    def update_tiles(self):
        ...
        for y in range(gridSizeY):
            for x in range(gridSizeX):
                ...
                new_tile = Tile(screen_x//4 + screen_y//2 + (x-gridSizeX//2)*TILE_SIZE , screen_y//2 - screen_x//4 + (y-gridSizeY//2)*TILE_SIZE)
                ...
                self.isoCoordTable[(x, y)]  =  [self.iso_coord(new_tile.rect.center[0], new_tile.rect.center[1])[0]+self.offsetx*TILE_SIZE  ,  self.iso_coord(new_tile.rect.center[0], new_tile.rect.center[1])[1]+self.offsety*TILE_SIZE]
                self.screen.blit(pygame.transform.scale_by(new_tile.image,self.zoom), self.isoCoordTable[(x,y)])

    def update_bobs(self):
        TILE_SIZE = 16*self.zoom
        BOB_SIZE = 16*self.zoom
        for key in self.Game.gridBob:
            for bob in self.Game.gridBob[key]:
                (x,y)=self.isoCoordTable[key]
                x+=BOB_SIZE/2 #centrer le bob sur la case
                y+=BOB_SIZE/5 #mettre le bob à 4/5 de la hauteur de la case
                self.screen.blit(bob.image_bob, (x,y))
