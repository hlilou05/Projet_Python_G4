...
def display_tiles(offx, offy):
    ...
    for y in range(-value // 2, value // 2):
        for x in range(-value // 2, value // 2):
            ...
            screen.blit(pygame.transform.scale(new_tile.image, (int(32 * zoom), int(32 * zoom))),
                        (iso_coord(new_tile.rect.x, new_tile.rect.y)[0] + offx * TILE_SIZE,
                         iso_coord(new_tile.rect.x, new_tile.rect.y)[1] + offy * TILE_SIZE))
