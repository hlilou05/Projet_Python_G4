...
class World():
    def __init__(self) :
        ...
        #self.save = SaveLoadSystem(".pkl", os.path.abspath("__pycache__"))
    ...
    def foodSpawn(self):
        ...
        """
        for _ in range(self.options["foodQty"]):
            x = randint(0, gridSizeX-1)
            y = randint(0, gridSizeY-1)
    ...
    def load_game(self):
        save_system = SaveLoadSystem(".sav", "saves")
        loaded_data = save_system.load_game_data(["gridBob", "gridFood", "bobcount", "dayCounter", "tickCounter"], [None, None, 0, 0, 0])
        if loaded_data[0] is not None:
            self.gridBob = loaded_data[0]
            self.gridFood = loaded_data[1]
            self.bobcount = loaded_data[2]
            self.dayCounter = loaded_data[3]
            self.tickCounter = loaded_data[4]
    ...
    def save_game(self):
        save_system = SaveLoadSystem(".sav", "saves")
        save_system.save_game_data([self.gridBob, self.gridFood, self.bobcount, self.dayCounter, self.tickCounter],["gridBob", "gridFood", "bobcount", "dayCounter", "tickCounter"])
    
    def auto_save(self):
        self.save.save_data(self, "sauvegarde")
        return
