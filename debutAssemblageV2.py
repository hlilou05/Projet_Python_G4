...
class View:
    ...
    def __init__(self, Game):
        ...
        self.display_tiles()
        ThreadDisplay= Thread(target=self.ContinuousDisplay(), daemon=True) #On consacre un Thread séparé pour raffraichir l'affichage en continu pendant toute la durée de l'éxecution.
    ...
    def makeBobArray(self):
        ...
        for key in self.Game.gridBob:
            for bob in self.Game.gridBob[key]:
                self.bobArray.append(bob)
                bob.coord=key
                bob.rect.center = bob.coord #update coord here if needed ?
        return

    def makeFoodArray(self):
        return

    def display_bobs(self):
        for bob in self.bobArray:
            location = bob.rect.center
            self.screen.blit(pygame.transform.scale_by(bob.image,self.zoom), (self.iso_coord(new_tile.rect.x, new_tile.rect.y)[0]+self.offsetx*TILE_SIZE,self.iso_coord(new_tile.rect.x, new_tile.rect.y)[1]+self.offsety*TILE_SIZE))
...
affichage = View(GameWorld)

affichage.ContinuousDisplay()
